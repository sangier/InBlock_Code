...
function delegatePrefix(bytes16 _parent_ip, bytes16 del_ip, uint8 bar, address delegated_address) notStopped() public {
	...
	int id=int(reverseSparse(_parent_ip));
...
function revokeDelegatedPrefix(bytes16 parent_ip, bytes16 del_ip,uint8 mask) notStopped() public {	
			
			int id=int(reverseSparse(parent_ip));
...
//To check the consistance of delegatePrefixs
...
function getDelBlockID(int id, int id2) view public returns(bytes16, address,uint8,int,int,int){
...
function getIDsDelBlocks(bytes16 ip, int start, int stop)view public returns(uint[]result){
...
function getDelegatedPrefixPolicyURI(bytes16 ip_parent, bytes16 ip, uint8 mask)view public returns(bytes,bytes,bytes){
	
			int id= int(reverseSparse(ip_parent));
			require(isPrefixInUse(id));
			int id2= del_find(ip_parent, ip, mask);
			require(id2!=-1 && id2!=-2);
			return (blocks[id].del_blocks[id2].info.uri,blocks[id].del_blocks[id2].info.hash,blocks[id].del_blocks[id2].info.hashFunction);
