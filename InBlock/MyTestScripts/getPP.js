...
function timeConverter(UNIX_timestamp){
  ...
  for (let i = 0; i < 1; i++) {
    SimpleStorage.deployed().then(function(instance) {
		...
		console.log(now);
        return instancee.getPrefixCost();
		...
		//console.log(t)
		let tobesaved="getPrefixCost,"+result.tx+","+t+","+result.receipt.gasUsed+"\n";
		fs.appendFile('DataGPC.txt', tobesaved, (err) => {  
    // throws an error, you could also catch it here
    if (err) throw err;

    // success case, the file was saved
    console.log('getPrefixCost Data saved!');
});
