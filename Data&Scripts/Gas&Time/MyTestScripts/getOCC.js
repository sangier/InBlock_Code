...
function timeConverter(UNIX_timestamp){
  ...
  for (let i = 0; i < 1; i++) {
    SimpleStorage.deployed().then(function(instance) {
		...
		console.log(now);
        return instancee.getOracleCurrencyConversion({value:12000000000000000});
		...
		//console.log(t)
		let tobesaved="getOracleCurrencyConversion,"+result.tx+","+t+","+result.receipt.gasUsed+"\n";
		fs.appendFile('DatagOCC.txt', tobesaved, (err) => {  
    // throws an error, you could also catch it here
    if (err) throw err;

    // success case, the file was saved
    console.log('getOracleCurrencyConversion Data saved!');
});
